...
## A8. v0.2 — app-server backend + standalone runner (user directive: "steal from them"; ideas adapted from scasella/claude-dynamic-workflows-codex, MIT-licensed concepts reimplemented from our own protocol grounding, not copied code)
...
1. **`runner/app-server-client.js`** — zero-dependency Node ≥18 client. EXACT API contract (both build tasks code against this, enabling parallel build):
...
**Credit:** README acknowledges scasella/claude-dynamic-workflows-codex as the architectural inspiration for the app-server backend and sessionful workers.
