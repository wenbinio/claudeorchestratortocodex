...
## A10. v0.4 — delegate mode (user pivot: "I don't need the trust pipeline. I want better UI, better UX — in-Claude transcripts on par with Opus/Sonnet agents — and better steering from Claude agents")

The trust pipeline (worktrees, adversarial review, verdict gating, stage ladder) remains for `dispatch`/`solo` users but is NOT the daily driver. Delegate mode is a first-class live-delegation experience:

1. **`runner/delegate.mjs`** — live-session runner built directly on the vendored `codexSession.js` (its `session.steer()` is the point). Behavior: start a persistent thread with cwd = the USER'S working tree by default (`--isolated` opts into a worktree); stream every notification to `<runDir>/events.jsonl` AS IT ARRIVES (never buffered to end); write `<runDir>/state.json` (threadId, pid, status, lastActivityAt, tokens) on every change; poll `<runDir>/steer.inbox.jsonl` — a line appended by `--steer` — and apply it: `turn/steer` when a turn is active, else a follow-up `turn/start` on the same warm thread. Modes: `delegate.mjs --start <task.json>`, `--steer <runDir> "<text>"`, `--status <runDir>`, `--interrupt <runDir>`. The steer/status/interrupt commands act through the run dir (inbox/state files), NOT cross-process JSON-RPC — the owning process is the only one talking to the app-server. Verification is ADVISORY: if a verify command is configured, run it after each turn settles and record the result in state.json + events; never park, never gate, never touch git beyond reporting `git status`/diff stats.
2. **`skills/delegate/SKILL.md`** — the in-Claude UX. The session spawns ONE narrator wrapper agent (`codex-fleet:codex-driver` type or general): it launches delegate.mjs --start in the background, then loops — read NEW lines from events.jsonl, narrate each meaningful beat in its own words ("Codex is reading tests/…", "patched slug.py", "verify: 4 passed"), check for steering instructions relayed from the main session, forward them via --steer — and finishes with a diff summary + advisory verify result. The wrapper's live tool-call/narration stream in the harness panel IS the Opus-parity transcript. The main session relays user steering ("tell it to use dataclasses") to the wrapper or directly via --steer. Honest ceiling documented: narration-beat parity yes; token-level streaming of Codex output into the panel is a harness surface we cannot reach.
3. README repositions delegate as the everyday path alongside solo; solo remains the "verified + reviewed single task" middle tier; dispatch the batch tier.
...
- **verify quoting on Windows (MINOR, documented gotcha):** the runner executes each `verify` string through the platform shell (PowerShell on Windows, `sh -c` on POSIX). A verify that begins with a quoted executable path needs the PowerShell call operator (`& "C:\...\tool.exe" ...`); the same `&` must NOT appear on POSIX (it backgrounds). The dispatch skill should tell users to write platform-appropriate verify strings; a future robustness pass could auto-detect. Surfaced by the v0.3 mock harness.
...
## A11. v0.4.1 — deferred-backlog sweep (Claude-agent workflow, timeboxed)

One pass through the logged deferrals (A9 "out of scope" + A10 Wave 1 follow-ups), each shipped as the smallest correct change:

1. **Long-turn continuation restored in both transports.** On turn timeout, instead of killing outright, each transport grants ONE continuation of half the original window, then enforces the kill path. Applies to app-server and exec alike.
2. **exec prompt via stdin / temp file** (closes A10 Wave 1 MAJOR). `runner/transports/exec.mjs` no longer passes the prompt as one shell argument, so the Windows ~32 KB command-line cap can't truncate a large spec + correction tail.
3. **`fleet-runner --resume`** (A9 deferral, Fable #3). Re-running a batch skips tasks whose recorded result is `done` AND whose branch tip still matches the recorded commit SHA; non-done tasks get sanctioned debris removal (their stale worktree/branch are cleaned) before re-dispatch. Crash-interrupted runs become re-runnable instead of hand-salvage jobs.
4. **`resolveBaseSha` subprocess timeout.** The one remaining unbounded git subprocess in the CLI adapter gets an explicit timeout, so a hung repo can't stall the whole pool before it starts.
5. **Commit provenance trailers** (A9 deferral, Fable #7). Every fleet commit carries `Codex-Fleet-Task:` and `Codex-Session:` trailers — fleet-log + git history become a bidirectionally traceable audit trail (log entry → commit, commit → worker session/transcript).
